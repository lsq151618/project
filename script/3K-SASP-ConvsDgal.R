...
## 1. Data Loading
data_dir <- "./"
df <- as.data.frame(read_excel(file.path(data_dir, "ConvsDgal.xlsx"), sheet = 1))
rownames(df) <- df[[1]]
...
sample_order <- c("D-gal1", "D-gal2", "D-gal3", "Ctrl1", "Ctrl2", "Ctrl3")
...
## 5. Execution
cairo_pdf(file.path(data_dir, "SASP-ConvsDgal.pdf"), width = 7, height = 7, pointsize = 10.5)
draw_circos(); dev.off()
png(file.path(data_dir, "SASP-ConvsDgal.png"), width = 2100, height = 2100, res = 300, pointsize = 10.5)
draw_circos(); dev.off()